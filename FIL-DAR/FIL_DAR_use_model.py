import spacy
import pandas as pd
from FIL_DAR_custom_component import custom_translator  # Ensure the component is registered
from FIL_DAR_custom_component import normalize_text  # Import normalize_text function


# Load the translations from the XLSX file
file_path = "FIL-DAR.xlsx"
...
for index, row in df.iterrows():
    if isinstance(row['Tagalog'], str) and isinstance(row['Bikol'], str):
        tagalog_phrase = normalize_text(row['Tagalog'])
        bikol_phrase = normalize_text(row['Bikol'])
...
def translate_sentence(sentence):
    nlp_loaded = spacy.load("FIL-DAR_translator.model")  # Load the model
